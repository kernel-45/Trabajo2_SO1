...
int internal_fg(char **args){
    ...
    if (args[1] == NULL) {
        fprintf(stderr, ROJO_T "Error de sintaxis, no has indicado el proceso"); 
        return 0; 
    } else {
            int posicionTabla = atoi(args[1]);
                if ((posicionTabla > n_job || posicionTabla == 0))
    {
        fprintf(stderr, ROJO_T "no existe el trabajo %d en la tabla\n" RESET, posicionTabla); 
        return 0; 
    }
        if (jobs_list[posicionTabla].estado == 'D')
        {
             kill(jobs_list[posicionTabla].pid, SIGCONT); 
        }
        //Ahora si encontramos un & tenemos que quitarlo
        if (jobs_list[posicionTabla].cmd[srtlen(jobs_list[posicionTabla].cmd)-1] == '&')
        {
            jobs_list[posicionTabla].cmd[srtlen(jobs_list[posicionTabla].cmd)-1] == '\0'; 
        }
        actualizar_job(0, jobs_list[posicionTabla].pid, jobs_list[posicionTabla].estado, jobs_list[posicionTabla].cmd);
        fprintf("%s", jobs_list[0].cmd) ; 
        jobs_list_remove(posicionTabla); 
        while (jobs_list[0].pid > 0){
            pause(); 
        }
        return 1; 
...
int internal_bg(char **args)
{
    ...
    if (jobs_list[posicionTabla].estado == 'E')
    {
        fprintf(stderr, ROJO_T "Error, el proceso ya estaba en segundo plano\n"RESET ); 
        return 0; 
    } else {
        if (DEBUG6)
    {
        fprintf(stderr, GRIS_T "[internal_bg()→ señal 18 enviada a %d (%s)]\n" RESET, jobs_list[posicionTabla].pid, jobs_list[posicionTabla].cmd);
    }
        jobs_list[posicionTabla].estado = 'E'; 
        jobs_list[posicionTabla].cmd[strlen(jobs_list[posicionTabla].cmd -1)] = '&'; 
        kill(jobs_list[posicionTabla].pid, SIGCONT); 
        return 1; 
    }   
}
int is_output_redirection (char **args){
    int puntero = 0; 
    while ((args[puntero] != NULL) && (strcmp(args[puntero], ">") != 0))
    {
        puntero++; 
    }
    if(args[puntero] == NULL){
        return 0; 
    }
    args[puntero] = NULL; //Así lo requiere el execvp
    if (args[puntero +1] == NULL){
        fprintf(stderr, ROJO_T "Error de sintaxis, falta poner el nombre del fichero\n" RESET); 
        return 0; 
    }
       int fd; 
    if ((fd = open(args[puntero + 1], O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR)) < 0)
    {
